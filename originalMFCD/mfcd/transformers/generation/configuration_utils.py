...
class GenerationMode(ExplicitEnum):
    ...
    MULTI_FREQUENCY_CONTRASTIVE_DECODING = "multi_frequency_contrastive_decoding"
...
class GenerationConfig(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        # MFC-Decode Config
        self.mfc_high_alpha = kwargs.pop("mfc_high_alpha", None)
        self.mfc_low_alpha = kwargs.pop("mfc_low_alpha", None)
        self.mfc_beta = kwargs.pop("mfc_beta", None)
        self.mfc_jsd = kwargs.pop("mfc_jsd", False)
        self.mfc_entropy = kwargs.pop("mfc_entropy", False)
    ...
    def get_generation_mode(self, assistant_model: Optional["PreTrainedModel"] = None) -> GenerationMode:
        ...
        # Multi-Frequency Contrastive Generation
        if self.mfc_high_alpha is not None or self.mfc_low_alpha is not None or self.mfc_beta is not None:
            if self.mfc_high_alpha is None or self.mfc_low_alpha is None or self.mfc_beta is None:
                raise ValueError(
                    "`mfc_high_alpha` and `mfc_low_alpha` and `mfc_bata` must be set together"
                )
            elif generation_mode not in (GenerationMode.GREEDY_SEARCH, GenerationMode.SAMPLE):
                raise ValueError(
                    "You've set `mfc_high_alpha` and `mfc_low_alpha` and `mfc_bata`, which triggers "
                    "`Multi-Frequency Contrastive Generation`. Currently, `Multi-Frequency Contrastive Generation` "
                    "is only supported with Greedy Search and Sample."
                )
            else:
                generation_mode = GenerationMode.MULTI_FREQUENCY_CONTRASTIVE_DECODING
