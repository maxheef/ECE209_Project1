...
class GenerationMixin:
    ...
    def _prepare_generation_config(
            self, generation_config: Optional[GenerationConfig], use_model_defaults: Optional[bool] = None,
            **kwargs: Dict
    ...
    def generate(
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
        ...
        elif generation_mode == GenerationMode.MULTI_FREQUENCY_CONTRASTIVE_DECODING:
            # 11. expand input_ids with `num_return_sequences` additional sequences per batch
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids=input_ids,
                expand_size=generation_config.num_return_sequences,
            ...
            # 12. run `Multi-Frequency Contrastive Generation`
            result = self._multi_frequency_contrastive_generation(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
    ...
    def _multi_frequency_contrastive_generation(
            self,
            input_ids: torch.LongTensor,
            logits_processor: LogitsProcessorList,
        ...
        r"""
        Generates sequences of token ids for models with a language modeling head using **multi-frequency contrastive
        generation** and can be used for vision-to-text models.
        ...
        # check for multi-frequency contrastive generation
        assert input_ids.dim() == 2 and input_ids.size(0) % 3 == 0, (
            "`Language Contrastive Decoding` only support 2D input_ids composed of original inputs and inputs with "
            "high-pass images and inputs with low-pass images."
        )
        assert return_dict_in_generate is None or return_dict_in_generate is False, (
            "`Language Contrastive Decoding` only supports `return_dict_in_generate=True`"
        )

        # init values for multi-frequency contrastive generation
        mfc_high_alpha = generation_config.mfc_high_alpha
        mfc_low_alpha = generation_config.mfc_low_alpha
        mfc_beta = generation_config.mfc_beta
        mfc_jsd = generation_config.mfc_jsd
        mfc_entropy = generation_config.mfc_entropy
        mfc_batch_size = input_ids.size(0) // 3
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # apply mfc
            next_token_logits[0:mfc_batch_size, :] = self._apply_multi_frequency_contrastive(
                next_token_logits[0:mfc_batch_size, :],
                next_token_logits[mfc_batch_size:mfc_batch_size * 2, :],
                next_token_logits[mfc_batch_size * 2:mfc_batch_size * 3, :],
                mfc_high_alpha,
                mfc_low_alpha,
                mfc_beta,
                mfc_entropy,
            ...
            # make the input for contrastive synchronous with the original input
            next_tokens[mfc_batch_size:mfc_batch_size * 2] = next_tokens[0:mfc_batch_size]
            next_tokens[mfc_batch_size * 2:mfc_batch_size * 3] = next_tokens[0:mfc_batch_size]
        ...
        return input_ids[0:mfc_batch_size]

    def _apply_multi_frequency_contrastive(
            self,
            original_logits: torch.FloatTensor,
            high_pass_logits: torch.FloatTensor,
            low_pass_logits: torch.FloatTensor,
            mfc_high_alpha: float,
            mfc_low_alpha: float,
            mfc_beta: float,
            mfc_entropy: bool = False,
            mfc_jsd: bool = False,
    ) -> torch.FloatTensor:
        """
        Apply `Multi-Frequency Contrastive` to logits of next token.
        Args:
            original_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
                Logits of next token, generate with original images.
            high_pass_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
                Logits of next token, generate with high-pass images.
            low_pass_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
                Logits of next token, generate with low-pass images.
            mfc_high_alpha (`float`):
                Argument `high_alpha` of `Multi-Frequency Contrastive`.
            mfc_low_alpha (`float`):
                Argument `low_alpha` of `Multi-Frequency Contrastive`.
            mfc_beta (`float`):
                Argument `beta` of `Multi-Frequency Contrastive`.
        Returns:
            `torch.FloatTensor` of shape `(batch_size, sequence_length)`:
            Logits of next token in `Multi-Frequency Contrastive`.
        """
        # probability of next tokens
        original_p = F.softmax(
            original_logits,
            dim=-1,
        )

        # The probability of the token with the maximum probability
        original_max_p = original_p.max(
            dim=-1,
            keepdim=True
        ).values

        mfc_high_alpha = torch.tensor(
            data=mfc_high_alpha,
            dtype=original_logits.dtype,
            device=original_logits.device
        )
        mfc_low_alpha = torch.tensor(
            data=mfc_low_alpha,
            dtype=original_logits.dtype,
            device=original_logits.device
        )

        mfc_beta = torch.tensor(
            data=mfc_beta,
            dtype=original_p.dtype,
            device=original_p.device,
        )
        ...
        # Mask out the tokens whose probabilities in the token probability distribution are less than beta times the
        # probability of the token with the highest probability.
        mfc_mask = original_p.lt(
            other=mfc_beta.unsqueeze(dim=-1) * original_max_p,
        )

        if mfc_jsd:
            high_jsd = self.jsd(original_logits, high_pass_logits)
            low_jsd = self.jsd(original_logits, low_pass_logits)

            mfc_high_alpha = mfc_high_alpha * torch.exp(-high_jsd)
            mfc_low_alpha = mfc_low_alpha * torch.exp(-low_jsd)

        mfc_logits = (
                (1 + mfc_high_alpha + mfc_low_alpha) * original_logits
                - mfc_low_alpha * low_pass_logits
                - mfc_high_alpha * high_pass_logits
        )

        # Mask out the tokens whose probabilities in the token probability distribution are less than beta times
        # the probability of the token with the highest probability.
        mfc_logits[mfc_mask] = torch.finfo(mfc_logits.dtype).min
    ...
    def jsd(self, logits1, logits2):
        ...
        kl1 = F.kl_div(torch.log(p1), m, reduction='batchmean')
        kl2 = F.kl_div(torch.log(p2), m, reduction='batchmean')

        d = torch.sqrt(0.5 * (kl1 + kl2))
