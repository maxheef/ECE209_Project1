...
def run_cmd(cmd, cwd=None, quiet=True):
    ...
    # We use capture_output=True if quiet to store logs in case of failure
    if quiet:
...
def setup_hardware():
    ...
    # Hardware setup scripts usually have their own internal logging; 
    # if you want them silenced, you'd need to modify them too.
    if "H100" in gpu_name:
...
def setup_datasets():
    ...
    # Using quiet=True here suppresses the wget progress bar
